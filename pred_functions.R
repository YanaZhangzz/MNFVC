...
gamma = function(Y,X,Tpoints,W,esti_coef,t_est){
  ...
  for (m in 1:num.points){
    ...
    Y_Ker_mat_1 = Y1s*Ker_mat/ns/fs#Y*Ker_mat1[i][j]/ns1[i]/fs1[i] #100*50
    Y2_Ker_mat_1 = Y1s^2*Ker_mat/ns/fs#100*50,N*M
...
lk = function(K,Y,X,Tpoints,W,esti_coef,t,ecov11,ecov22){
  ...
  for(m in 1:num.points){
    ...
    T_diff = Tpoints - t[m]#N*numpointsά
    h = rule.thumb(T_diff[1,])/3
    Ker_mat = gaussian.kernel(T_diff, h)
...
pred = function(dat)
{
  ...
  t_grid <- seq(0, 1, length.out = num.points)
  train_grid <- seq(1, max.points, by = 2)
  test_grid <- seq(2, max.points, by = 2)  
  ...
  model = est_d_beta(train_Y,X,train_Tpoints,W,D0,B0,Sige1,num.points)
  ecovp = est_covariance(model$est_coef,model$timeline,test_Y1,test_Y2,X,test_Tpoints,W)
  ecov11p = ecovp$cov11
  ecov22p = ecovp$cov22
  ...
  for(K in 1:num.points){
    BIC1[K]=obBIC(K,test_Y,X,test_Tpoints,W,model$est_coef,model$timeline,ecov11p,ecov22p)
  }
  K = which.min(BIC1)
  
  gammap = gamma(test_Y,X,test_Tpoints,W,model$est_coef,model$timeline)
  sci1 = matrix(0,nrow=N,ncol=K)
  sci2 = matrix(0,nrow=N,ncol=K)
  for(i in 1:(N)){
    for(k in 1:K){
  ...
  phisum = matrix(0,nrow=(N*2),ncol=num.points)
  for (m in 1:num.points){
    for (k in 1:K){
  ...
  num = length(test_grid)
  
  Ymat = matrix(0,nrow=2*N,ncol=num)
  for(m in 1:num){
  ...
  IMSE = sum(crossprod(Yhat - test_Y)) / (2 * nrow(Yhat)*(num.points))

  return(list(model=model,IMSE=IMSE))
}

inside_pred = function(model)
{
  Y1 = dat$Y1
  Y2 = dat$Y2
  X = dat$v1
  ...
  ecovp = est_covariance(model$est_coef,model$timeline,train_Y1,train_Y2,X,train_Tpoints,W)
  ecov11p= ecovp$cov11
  ecov22p = ecovp$cov22
  e1evaluep = eigen(ecov11p)$values
  e1efunp = eigen(ecov11p)$vectors
  ...
  obBIC = function(K,Y,X,Tpoints,W,esti_coef,t,ecov11,ecov22){
    return(-2*lk(K,Y,X,Tpoints,W,esti_coef,t,ecov11,ecov22)+8*log(N*4/5))
  }
  BIC1=c()
  for(K in 1:num.points){
    BIC1[K]=obBIC(K,train_Y,X,train_Tpoints,W,model$est_coef,model$timeline,ecov11p,ecov22p)
  }
  K = which.min(BIC1)
  
  gammap = gamma(train_Y,X,train_Tpoints,W,model$est_coef,model$timeline)
  sci1 = matrix(0,nrow=N,ncol=K)
  sci2 = matrix(0,nrow=N,ncol=K)
  for(i in 1:(N)){
    for(k in 1:K){
  ...
  b11 = model$est_coef[,5]
  b12 = model$est_coef[,7]
  b21 = model$est_coef[,6]
  b22 = model$est_coef[,8]
  ...
  IMSE = sum(crossprod(Yhat- train_Y)) / (2 * nrow(Yhat)*(num.points))

  return(list(IMSE=IMSE))
}
...
pred_NFVC=function(dat){
  ...
  NFVC_ans1 = est_entire_NFVC(Y=train_Y1,X,Tpoints=train_Tpoints,W,num)
  NFVC_ans2 = est_entire_NFVC(Y=train_Y2,X,Tpoints=train_Tpoints,W,num)
  ...
  ecovp11 = est_cov_NFVC(NFVC_ans1$est_coef,NFVC_ans1$timeline,test_Y1,X,test_Tpoints,W)
  ecovp22 = est_cov_NFVC(NFVC_ans2$est_coef,NFVC_ans2$timeline,test_Y2,X,test_Tpoints,W)
  ecov11p = ecovp11
  ecov22p = ecovp22
  e1evaluep = eigen(ecov11p)$values
  e1efunp = eigen(ecov11p)$vectors
  ...
  BIC1=c()
  for(K in 1:num){
    BIC1[K]=obBIC(K,NFVC_ans1$timeline,NFVC_ans1$est_coef,test_Y1,X,test_Tpoints,W,ecov11p)
  }
  K1 = which.min(BIC1)
  
  BIC2=c()
  for(K in 1:num){
    BIC2[K]=obBIC(K,NFVC_ans2$timeline,NFVC_ans2$est_coef,test_Y2,X,test_Tpoints,W,ecov22p)
  }
  K2 = which.min(BIC2)
  
  gammap = gammap_NFVC(NFVC_ans1,NFVC_ans2,NFVC_ans1$timeline,test_Y,X,test_Tpoints,W)
  
  sci1 = matrix(0,nrow=N,ncol=K1)
  sci2 = matrix(0,nrow=N,ncol=K2)
  for(i in 1:(N)){
    for(k in 1:K1){
      sci1[i,k] = e1evaluep[k]*t(e1efunp[,k])%*%solve(ecov11p)%*%gammap[i,1,]
    }
  ...
  phisum = matrix(0,nrow=(N*2),ncol=num)
  for (m in 1:num){
    for (k in 1:K1){
      phi1 = sci1[,k]*e1efunp[k,m]
    }
  ...
  right.mat1 = v1%*%beta.seq1+phisum[(1:N),]
  right.mat2 = v1%*%beta.seq2+phisum[-(1:N),]
  Y1 = right.mat1+rho.mat11*(W%*%right.mat1)+(rho.mat11^2)*
    (W%*%W%*%right.mat1)+(rho.mat11^3)*(W%*%W%*%W%*%right.mat1)
  Y2 = right.mat2+rho.mat22*(W%*%right.mat2)+(rho.mat22^2)*
    (W%*%W%*%right.mat2)+(rho.mat22^3)*(W%*%W%*%W%*%right.mat2)
  Y.all=rbind(Y1,Y2)
  
  IMSE = sum(crossprod(Y.all - test_Y)) / (2 * nrow(Y.all)*num)

   return(list(NFVC_ans1=NFVC_ans1,NFVC_ans2=NFVC_ans2,IMSE=IMSE))
}


inside_pred_NFVC=function(NFVC_ans1,NFVC_ans2){
  ...
  ecovp11 = est_cov_NFVC(NFVC_ans1$est_coef,NFVC_ans1$timeline,train_Y1,X,train_Tpoints,W)
  ecovp22 = est_cov_NFVC(NFVC_ans2$est_coef,NFVC_ans2$timeline,train_Y2,X,train_Tpoints,W)
  ecov11p = ecovp11
  ecov22p = ecovp22
  e1evaluep = eigen(ecov11p)$values
  e1efunp = eigen(ecov11p)$vectors
  ...
  obBIC = function(K,t_est,esti_coef,Y,X,Tpoints,W,ecov11){
    return(-2*loss_func(K,t_est,esti_coef,Y,X,Tpoints,W,ecov11)+K*((N*4/5)^0.8))
  }
  BIC1=c()
  for(K in 1:num){
    BIC1[K]=obBIC(K,NFVC_ans1$timeline,NFVC_ans1$est_coef,train_Y1,X,train_Tpoints,W,ecov11p)
  }
  K1 = which.min(BIC1)
  
  BIC2=c()
  for(K in 1:num){
    BIC2[K]=obBIC(K,NFVC_ans2$timeline,NFVC_ans2$est_coef,train_Y2,X,train_Tpoints,W,ecov22p)
  }
  K2 = which.min(BIC2)
  
  
  gammap = gammap_NFVC(NFVC_ans1,NFVC_ans2,NFVC_ans1$timeline,train_Y,X,train_Tpoints,W)
  ...
  IMSE = sum(crossprod(Y.all - train_Y)) / (2 * nrow(Y.all)*num)
  
  return(list(IMSE=IMSE,R2=R2))
}
...
pred_Zhu = function(dat){
  ...
  t=seq(0,1,length=num)
  g00 = g0[(1:N),]
  esti = iteration(3,1e-5,train_Y,X,z,g00)
  ans=esti$alpha
  beta=esti$beta
  bns=est_abt(test_Y,X,z,ans,beta)
  eta = est_eta(test_Y,X,z,t,ans,bns,beta)
  
  ecovp = est_covt(test_Y,X,z,t,ans,bns,beta)
  ecov11p = ecovp$cov11
  ecov22p = ecovp$cov22
  e1evaluep = eigen(ecov11p)$values
  e1efunp = eigen(ecov11p)$vectors
  ...
  sci1 =matrix(0,nrow=N,ncol=num)
  for(m in 1:num){
    sci1 = sci1+eta[,1,m]%*%t(e1efunp[,m])*(1/num)
  }
  sci2 =matrix(0,nrow=N,ncol=num)
  for(m in 1:num){
    sci2 = sci2+eta[,2,m]%*%t(e2efunp[,m])*(1/num)
  }
  ...
  Y1 = X%*%t(ans[,1:2])+bns[,1]+phisum[(1:N),]
  Y2 = X%*%t(ans[,3:4])+bns[,2]+phisum[-(1:(N)),]
  
  Y.all=rbind(Y1,Y2)
  
  IMSE = sum(crossprod(Y.all - test_Y)) / (2 * nrow(Y.all)*num)
 
  return(list(esti=esti,IMSE=IMSE2))
 
}

inside_pred_Zhu = function(esti){
  ...
  num.points=ncol(Y1)
  t_grid <- seq(0, 1, length.out = num.points)
  train_grid <- seq(1, num.points, by = 2) 
  test_grid <- seq(2, num.points, by = 2) 
  ...
  ans=esti$alpha
  beta=esti$beta
  bns=est_abt(train_Y,X,z,ans,beta)
  eta = est_eta(train_Y,X,z,t,ans,bns,beta)
  
  ecovp = est_covt(train_Y,X,z,t,ans,bns,beta)
  ecov11p = ecovp$cov11
  ecov22p = ecovp$cov22
  e1evaluep = eigen(ecov11p)$values
  e1efunp = eigen(ecov11p)$vectors
